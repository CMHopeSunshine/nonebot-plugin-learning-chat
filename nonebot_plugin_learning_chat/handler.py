...
class LearningChat:
    ...
    async def _learn(self) -> Result:
        # log_debug('群聊学习', f'收到来自群<m>{self.data.group_id}</m>的消息<m>{self.data.message}</m>')
    ...
    async def answer(self) -> Optional[List[Union[MessageSegment, str]]]:
        ...
        if result == Result.Ban:
            ...
            config_manager.save()
            log_info('群聊学习', f'群<m>{self.data.group_id}</m>{"开启" if result == Result.SetEnable else "关闭"}学习功能')
            ...
            if contexts := await ChatContext.filter(count__gte=config.answer_threshold).all():
                ...
                for context in contexts:
                    if (random.random() < config.speak_continuously_probability or not len(speak_list)) and len(
                        ...
                        break
                if speak_list:
                    # log_debug('群聊学习', f'主动发言：将向群<m>{group_id}</m>主动发言<m>{" ".join(speak_list)}</m>')
    ...
    async def _check_allow(self, message: Union[ChatMessage, ChatAnswer]) -> bool:
        ...
        if any(i in raw_message for i in
            ...
            # log_debug('群聊学习', f'➤检验<m>{keywords}</m><r>不通过</r>')
            return False
        ...
        if ban_word := await ChatBlackList.filter(keywords=message.keywords).first():
            if ban_word.global_ban or message.group_id in ban_word.ban_group_id:
                # log_debug('群聊学习', f'➤检验<m>{keywords}</m><r>不通过</r>')
                return False
        # log_debug('群聊学习', f'➤检验<m>{keywords}</m><g>通过</g>')
        return True
